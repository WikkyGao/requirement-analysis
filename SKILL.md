...
description: "多格式需求文档分析与整合。自动解析 Markdown、Excel、Word、思维导图文件，智能去重和补充需求信息，生成标准化的 PRD.md。适用场景：用户有多个需求来源需要整合、需要从零梳理需求、需要将模糊想法转化为结构化 PRD。触发词：需求分析、分析需求、整理需求、PRD、需求文档、需求梳理、梳理需求。"
---
...
1. **识别用户角色**: 从功能名推断系统角色（管理员/普通用户）
2. **推导业务流程**: 功能间时序关系 → 完整业务流程
3. **补全隐含功能**: 有"登录"无"退出" → 追问；有"退款"无"审批" → 追问
4. **追问边界条件**: "导出数据" → 格式/范围；"搜索" → 条件/方式
...
### Task 3: 智能整合（去重/冲突/补充）

**分工**: [LLM→Script]

- [ ] Step 1: [LLM 决策] **语义去重** — 基于语义相似度比较功能名称和描述的实质一致性
  - 高优先级文档保留为主记录
  - 合并所有字段信息
  - 记录合并来源
- [ ] Step 2: [Script 执行] **冲突检测** — 调用 `scripts/conflict_detector.py`：

```bash
echo '{"features_by_source": {"doc1.md": [...], "doc2.xlsx": [...]}}' | python scripts/conflict_detector.py
```
